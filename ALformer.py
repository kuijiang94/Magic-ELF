...
class stage_encoder_module_dn(nn.Module):
    def __init__(self, n_feat, kernel_size, reduction, bias, act):
        ...
        ffn_expansion_factor = 2.66
        LayerNorm_type = 'WithBias'  ## Other option 'BiasFree'
        self.former1 = nn.Sequential(*[RestormerBlock(dim=n_feat, num_heads=heads, ffn_expansion_factor=ffn_expansion_factor, bias=bias, LayerNorm_type=LayerNorm_type) for i in range(num_blocks)])
        self.former2 = nn.Sequential(*[RestormerBlock(dim=n_feat, num_heads=heads, ffn_expansion_factor=ffn_expansion_factor, bias=bias, LayerNorm_type=LayerNorm_type) for i in range(num_blocks)])
    ...
    def forward(self, x):
        en_CAB1_FEA = self.former1(x)
        #en_S2FB1_FEA = self.S2FB1(x, en_CAB1_FEA)
        en_CAB2_FEA = self.former2(en_CAB1_FEA)
...
class stage_decoder_module_dn(nn.Module):
    ...
    def forward(self, x, stage_skip1, stage_skip2, stage_skip3):
        ...
        de_CAB1_FEA = self.former1(de_S2FB0_FEA)
        #de_S2FB1_FEA = self.S2FB1(de_S2FB0_FEA, de_CAB1_FEA)
        de_CAB2_FEA = self.former2(de_CAB1_FEA)
