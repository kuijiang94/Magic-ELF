...
def crop_center(img, new_w, new_h):
    ...
    if len(img.shape) == 2:
        center_cropped_img = img[int(l):int(l+new_w), int(t):int(t+new_h)]
    else:
        center_cropped_img = img[int(l):int(l+new_w), int(t):int(t+new_h), ...]
...
def tensor2img(tensor, out_type=np.uint8, min_max=(0, 1), reverse_channel=True):
    ...
    if n_dim == 4:
        ...
        if reverse_channel:
            ...
            img_np = np.transpose(img_np[:, :, :], (1, 2, 0))  # CHW->HWC, YCbCr
    elif n_dim == 3:
        img_np = tensor.numpy()
        if reverse_channel:
            img_np = np.transpose(img_np[[2, 1, 0], :, :], (1, 2, 0))  # CHW->HWC, RGB->BGR
...
class SRMDPreprocessing(object):
    def __init__(self, scale, pca, random, para_input=10, kernel=21, noise=True, cuda=False,
                 sig=2.6, sig_min=0.2, sig_max=4.0, rate_iso=1.0, scaling=3, rate_cln=0.2, noise_high=0.08):
        self.encoder = PCAEncoder(pca, cuda=cuda)
        self.kernel_gen = BatchSRKernel(l=kernel, sig=sig, sig_min=sig_min, sig_max=sig_max,
                                        rate_iso=rate_iso, scaling=scaling)
    ...
    def __call__(self, hr_tensor, kernel=False):
        ...
        b_kernels = Variable(self.kernel_gen(self.random, B, tensor=True)).cuda() if self.cuda else\
                    Variable(self.kernel_gen(self.random, B, tensor=True))
        # blur
        if self.cuda:
...
if __name__ == '__main__':
    ...
    edge_detect = kornia.filters.Laplacian(kernel_size=5)
    img = cv2.imread('/home/xiyang/Projects/EDVR/imgs/00003.png')
    img = img / 255.
    img = torch.tensor(img, dtype=torch.float)
    img = img.permute(2, 0, 1).unsqueeze(0)

    # kernel = gauss_kernel(channels=3)
    # pyr = laplacian_pyramid(img, kernel, max_levels=3)
    # for p in pyr:
    #     p = tensor2img(p)
    #     plt.imshow(p)
    #     plt.show()
